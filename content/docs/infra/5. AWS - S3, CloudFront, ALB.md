...
ALB는 **L7(HTTP/HTTPS) 로드밸런서**예요. Session 3에서 Nginx가 하던 역할을 AWS가 관리해주는 서비스라고 생각하면 돼요.

- **Target Group:** ALB가 트래픽을 전달할 대상 (EC2 인스턴스들)
- **Listener:** 어떤 포트/프로토콜로 요청을 받을지 설정
- **Health Check:** Target의 상태를 주기적으로 확인. Session 1에서 만든 `/health` endpoint가 여기서 쓰여요!
...
- Session 4의 EC2에서 Nginx 컨테이너를 제거하세요. ALB가 그 역할을 대신합니다.
...
- `http://[EC2-IP]:3000/`과 `http://[EC2-IP]:3000/health`로 직접 접속이 되는지 확인하세요.
...
{{< alert context="info" text="CloudFront가 `/api/*` 경로를 ALB로 전달하려면, Node.js 서버에 `/api/` 접두사가 붙은 라우트가 필요해요. 기존 서버에 `/api/info` (OS 정보)와 `/api/health` 라우트를 추가하세요. 기존 `/health`는 ALB Health Check용으로 그대로 유지하세요." />}}
...
curl http://[CloudFront-Domain]/api/health
```
