...
- **AMI (Amazon Machine Image):** OS 이미지. Amazon Linux 2023 또는 Ubuntu 24.04 사용.
- **Key Pair:** SSH 접속용 키. 분실하면 인스턴스에 접속 불가!
- **Elastic IP:** 고정 공인 IP. 인스턴스를 중지/시작해도 IP가 변하지 않음.

{{< alert context="warning" text="**Free Tier 주의:** 2025년 7월 15일 이후 생성된 AWS 계정은 Free Tier 조건이 변경되었어요. t3.micro 외에도 t3.small, t4g.micro 등이 포함되지만, 기간이 12개월이 아닌 **6개월**로 줄었어요. 본인 계정의 Free Tier 조건을 반드시 확인하세요!<br>https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-free-tier-usage.html" />}}
...
# 앱 배포 (git clone 또는 compose.yaml 직접 작성)
git clone <your-repo>
cd my-server
docker compose up -d
...
{{< alert context="warning" text="2024년 2월부터 모든 Public IPv4 주소에 시간당 $0.005가 과금됩니다. EC2에 연결된 EIP도 예외가 아니에요! Free Tier 계정은 월 750시간이 무료이지만, 실습 후 사용하지 않는 EIP는 반드시 Release 해주세요.<br>https://aws.amazon.com/vpc/pricing/" />}}
