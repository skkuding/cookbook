...
- **Ingress:** 클러스터 외부에서 들어온 HTTP/HTTPS 요청을 도메인이나 경로에 따라 Service로 전달하는 규칙.
- **Ingress Controller:** Ingress 규칙을 실제로 읽고 트래픽을 전달하는 컴포넌트. Ingress 리소스만 생성해서는 동작하지 않아요. k3s에는 Traefik Ingress Controller가 기본으로 설치됩니다.
...
### (선택) Ingress로 서비스 노출하기

NodePort에서는 사용자가 EC2의 IP와 31000번 포트를 직접 알아야 했어요. 이번에는 Ingress를 사용하여 애플리케이션을 HTTP 기본 포트인 80번으로 노출해보세요.

k3s에는 Traefik Ingress Controller가 기본으로 설치되어 있습니다. 로컬 kind 환경에서 이 실습을 진행하려면 Ingress Controller를 별도로 설치해야 해요.

**요구사항:**

1. 현재 클러스터의 Ingress Controller와 IngressClass를 확인하세요.
2. `ingress.yaml`을 작성하세요.
3. `/`로 들어온 요청을 애플리케이션 Service로 전달하세요.
4. 경로의 `pathType`은 `Prefix`를 사용하세요.
5. Ingress를 적용하고 상태를 확인하세요.
6. EC2의 80번 포트로 `/`, `/api/info`, `/health`에 접속하세요.
7. NodePort와 Ingress의 요청 흐름 차이를 설명하세요.



```mermaid
flowchart LR
  Client -->|":31000"| NodePort["NodePort Service"]
  Client -->|":80 / 경로"| Controller["Traefik Ingress Controller"]
  Controller --> Rule["Ingress 규칙"]
...
| NodePort | Ingress |
|---|---|
| 포트로 Service를 노출 | HTTP 경로나 도메인으로 Service를 라우팅 |
| `EC2-IP:31000` | `EC2-IP/health` |
| TCP/UDP Service 방식 | HTTP/HTTPS 요청을 처리 |
| 별도의 Controller가 필요 없음 | Ingress Controller가 필요함 |

{{< alert context="info" text="Ingress는 라우팅 규칙이고 Traefik은 그 규칙을 실제로 처리하는 Ingress Controller예요. 두 개념을 구분하세요." />}}
