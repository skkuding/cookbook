...
저번 시간에는 HTML, CSS를 통해 웹의 구조와 디자인을 만드는 방법을 배웠어요. 이번 시간에는 JavaScript를 통해 웹의 동적인 기능을 만드는 방법을 배워볼게요!

## 공부할 내용 📚

### JavaScript

Javascript는 평소에 사용하던 C, JAVA와 비슷한 Syntax를 가지고 있어요! 덕분에 배우기 쉬울 거예요!
...
DOM(Document Object Model)은 HTML 문서의 구조를 트리 형태로 표현한 것이에요. 해당 챕터를 배우면 여러분들은 HTML 문서를 조작, 이벤트, 브라우저 API 등을 통해 더 다양한 웹 페이지를 만들 수 있어요!
...
ㄴ
위와 같이 디자인은 똑같지만, html 파일을 통해서 리스트를 만드는 것이 아니라 javascript 파일에서 포켓몬 데이터 배열을 **매핑**하여 리스트를 만들어요.

포켓몬 데이터는 `assets/pokemon.js`에 있어요. 이 데이터를 사용해서 메인페이지 리스트를 만들어 보아요!

### 2. 하나의 html 파일로 디테일 페이지 만들기

![디테일 페이지](../images/fe-4weak-2.png)

디테일 페이지도 디자인은 변함없어요! 다만 기존에는 포켓몬별로 디테일 페이지(html)를 만들었다면 이번에는 하나의 html 파일로 디테일 페이지를 만들어요.
